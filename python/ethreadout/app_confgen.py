...
def generate(
    ...
    queue_bare_specs = [
                           ...
                           for idx in range(NUMBER_OF_DATA_PRODUCERS)
                            ...
                            app.QueueSpec(inst=f"{FRONTEND_TYPE}_link_{idx}", kind='FollyMPMCQueue', capacity=100000)
                            for idx in range(NUMBER_OF_DATA_PRODUCERS)
                       ]
    ...
    confcmd = mrccmd("conf", "INITIAL", "CONFIGURED", [
        ...
        (f"vdreceiver", vdr.Conf(
            link_confs=[vdr.LinkConfiguration(
                source_port = 30000 + idx,
                queue_name = f"output_{idx}"
            ) for idx in range(NUMBER_OF_DATA_PRODUCERS)],
            ip = "192.168.10.101",
